def create_instance(nodes_dict):
    instance = {}
    for node, data in nodes_dict.items():
        adj_dict = {}
        for adj_node, weight in data['adjacents'].items():
            adj_dict[adj_node] = weight
        instance[node] = {'adjacents': adj_dict, 'weight': data['weight'], 'value': data['value']}
    return instance
